...
public class CameraView
{
	...
	/**
	 * Tekst wyswietlany nad miniaturkami wykonanych zdjec.
	 */
	public TextView miniPhotosText;
	...
	/**
	 * Layout w którym znajduje siê {@link #imgBig}. Ma ustawione t³o, a wyœwietlane zdjêcie
	 * zostaje nieco pomniejszone, przez co brzegi t³a widoczne s¹ jako ramka zdjêcia.
	 */
	public LinearLayout lBigPhotoBG;
	...
	private boolean dimensionsComputed;
...
//		app.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
		...
		for(int i = 0; i<MAX_PHOTOS; i++) 
		{
			...
			fileNames[i] = "dziura"+f.hashCode()+Integer.toString(i)+".jpg";
		...
		miniPhotosText = (TextView) app.findViewById(R.id.miniPhotosText);

		//wlaczenie aparatu
		preview = (SurfaceView) app.findViewById(R.id.surfaceViewCamera);
		//preview.setOnClickListener(new MyOnFotoClickListener(app)); a zeby nie mozna bylo robic zdjec przez nacisniecie podgladu
		...
		bDeletePhoto.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if(duze_foto != -1) {
					...
					if (photo.exists()) {
						photo.delete();
						app.wyswietlTekst("Zdjêcie usuniêto");
						pokazPodglad();
		...
		bUsunTagi.setOnClickListener(new OnClickListener() {
			public void onClick(View arg0) {
				...
				app.wyswietlTekst("Usuniêto znaczniki dodane na zdjêciu");
				showDeleteTagsButton();
			}
		});
	...
	private void setLayouts() {
		...
		lBigPhotoBG = (LinearLayout) app.findViewById(R.id.linLayBigPhotoBG);
	}
	...
	public void showPhotos()
	{
		...
		for(int i=0; i<fileNamesBool.length; i++) 
		{
			if(fileNamesBool[i] == true) 
			{
				...
				if (dimensionsComputed == false) {
					...
					int maxMiniH = (int) ((displayHeight - 35
							- app.tabWidget.getHeight() - miniPhotosText
							.getHeight()) / (double) MAX_PHOTOS);
					if(maxMiniH > 150)
					{
						maxMiniH = 150;
					}
					int maxMiniW = displayWidth - preview.getWidth() - 10
							- bTakePhoto.getWidth();
					imgViewW = maxMiniW;
					imgViewH = 0;
					while (true) {
		...
		{
			lMinPhotos.setVisibility(View.GONE);
		}
	}
}
