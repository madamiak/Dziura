			...
			public boolean onTouch(View arg0, MotionEvent arg1) 
			{
				...
				{ 
					...
					for(int i=0; i<dziuraAct.vCamera.tagList.size(); i++)
				    {
				    	...
				    	{
				    		if(arg1.getX() > tmp.getX() - 30 && arg1.getX() < tmp.getX() + 30
				    				&& arg1.getY() > tmp.getY() - 30 && arg1.getY() < tmp.getY() + 30)
				    		{
				    			dziuraAct.wyswietlTekst("Opis dotkniêtego znacznika: "+tmp.getText());
				    			czyDodacTag = false;
				    		}
				    	}
				    }
					if(czyDodacTag)
					{
						...
						if(getClickable())
						{
							...
							if(tagEditParams.height > 250)
							{
								tagEditParams.height = 250; //zeby pole nie bylo za duze
							}
