...
import projekt.zespolowy.dziura.AppView.AfterPhotoDialog;
...
public class MyOnFotoClickListener implements OnClickListener 
{
	...
	private SavingPhotoProgressBar savingBar;
	...
	public MyOnFotoClickListener(DziuraActivity dziuraActivity)
	{
		...
		savingBar = new SavingPhotoProgressBar(dziuraActivity);
	}
    	...
    	public void onPictureTaken(byte[] data, Camera camera)
    	{
    		for(int i = 0; i<dziuraAct.vCamera.fileNamesBool.length; i++)
    		{
    			if(dziuraAct.vCamera.fileNamesBool[i]==false)
    			{
        			...
        			{
        				if(dziuraAct.photoDialogShowed == false)
        				{
        					AfterPhotoDialog dialog = new AfterPhotoDialog(dziuraAct);
        					dialog.showDialog();
        					dziuraAct.photoDialogShowed = true;
        				}
        			}
        			break;
    			}
    ...
    class SavePhotoTask extends AsyncTask<byte[], String, String>
    {
    	...
    	protected String doInBackground(byte[]... jpeg)
    	{
			...
			else
			{
				...
				photo = new File(dziuraAct.getDir("photo", Context.MODE_WORLD_READABLE|Context.MODE_WORLD_WRITEABLE), dziuraAct.vCamera.fileNames[number]);
