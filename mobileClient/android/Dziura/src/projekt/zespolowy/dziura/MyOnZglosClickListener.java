...
public class MyOnZglosClickListener implements OnClickListener
{
	...
	/**
	 * Aktualne wspó³rzêdne geograficzne. Domyœlnie przybiera wartoœæ 0,0. Wspó³rzêdne ustalane
	 * przez {@link projekt.zespolowy.dziura.MyLocationListener}.
	 * 
	 * @see #currentLatitude
	 * @see #currentLongitude
	 */
	public double currentLatitude = 0.0, currentLongitude = 0.0;
	
	/**
	 * Obiekt konieczny do ustalenia pozycji GPS.
	 * @see android.location.LocationManager
	 */
	public LocationManager locationManager;
	
	/**
	 * Pasek postêpu wyœwietlany podczas ustalania lokacji GPS.
	 */
	public LocationProgressBar locProg;
	
	/**
	 * Pasek postêpu wyœwietlany podczas wysy³ania zg³oszenia.
	 */
	...
	public void onClick(View arg0)
	{
		if(sprawdzPola() == true)
		{
			if (dziuraAct.isInternetEnabled() == true)
			{
				...
				{
					locationManager = (LocationManager) dziuraAct
							.getSystemService(Context.LOCATION_SERVICE);
					if (!locationManager
							.isProviderEnabled(LocationManager.GPS_PROVIDER)) { //jesli GPS nie jest uruchomiony to pokazujemy okienko zeby wlaczyc GPSa
						dziuraAct.vOption.mOptionsView.postInvalidate();
						dziuraAct.sending = true;
						dziuraAct.showGpsOptions(
								"Aby pobraæ lokalizacjê, nale¿y uruchomiæ odbiornik GPS."
										+ " Czy chcesz go teraz w³¹czyæ?",
								"Uwaga");
					} else { //jesli jest wlaczony to pobieramy lokalizacje
						locProg = new LocationProgressBar(dziuraAct);
						locProg.run();
						GetLocationTask gpsLoc = new GetLocationTask(
								dziuraAct);
						gpsLoc.execute();
					}
	...
	public void showSuccesDialog() 
	{
		...
		else
		{
			...
			sendProg.run();
			SendingTask send = new SendingTask(dziuraAct, dziuraAct.sendUsingServer);
			send.execute();
		}
	}
