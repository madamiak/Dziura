...
/**
 * Klasa ta implementuje czesc menu dotyczaca aparatu. Umozliwia zmiane jego parametrow.
 */
public class CameraMenu
{
	...
	/**
	 * Inicjalizuje menu. Sprawdza, czy dany aparat wspiera kolejne opcje i jezeli wspiera, to dodaje
	 * dla nich odpowiednie opcje w menu.
	 * 
	 * @param menu obiekt {@link android.view.Menu}, do ktorego zostana przypisane elementu dotyczace aparatu
	 * @param camera obiekt aparatu, dla ktorego menu ma wprowadzac zmiany
	 * @return		<li>obiekt {@link android.view.Menu} zmodyfikowany o opcje zwiazane z aparatem
	 */
	public static Menu init(Menu menu, Camera camera)
	{
		...
		if(tmp != null)
		{
			SubMenu subFlash = menu.addSubMenu(0, MENU_CAMERA_FLASH, 1, R.string.menu_camera_flash);
			if(tmp.contains(Parameters.FLASH_MODE_AUTO))
				subFlash.add(0, MENU_CAMERA_FLASH_AUTO, 1, R.string.menu_camera_flash_auto);
			if(tmp.contains(Parameters.FLASH_MODE_OFF))
				subFlash.add(0, MENU_CAMERA_FLASH_OFF, 1, R.string.menu_camera_flash_off);
			if(tmp.contains(Parameters.FLASH_MODE_ON))
				subFlash.add(0, MENU_CAMERA_FLASH_ON, 1, R.string.menu_camera_flash_on);
			if(tmp.contains(Parameters.FLASH_MODE_RED_EYE))
				subFlash.add(0, MENU_CAMERA_FLASH_REDEYE, 1, R.string.menu_camera_flash_redeye);
			if(tmp.contains(Parameters.FLASH_MODE_TORCH))
				subFlash.add(0, MENU_CAMERA_FLASH_TORCH, 1, R.string.menu_camera_flash_torch);
		}
		tmp = camera.getParameters().getSupportedFocusModes();
		if(tmp != null)
		{
			SubMenu subFocus = menu.addSubMenu(0, MENU_CAMERA_FOCUS, 1, R.string.menu_camera_focus);
			if(tmp.contains(Parameters.FOCUS_MODE_AUTO))
				subFocus.add(0, MENU_CAMERA_FOCUS_AUTO, 1, R.string.menu_camera_focus_auto);
			if(tmp.contains(Parameters.FOCUS_MODE_FIXED))
				subFocus.add(0, MENU_CAMERA_FOCUS_FIXED, 1, R.string.menu_camera_focus_fixed);
			if(tmp.contains(Parameters.FOCUS_MODE_INFINITY))
				subFocus.add(0, MENU_CAMERA_FOCUS_INFINITY, 1, R.string.menu_camera_focus_infinity);
			if(tmp.contains(Parameters.FOCUS_MODE_MACRO))
				subFocus.add(0, MENU_CAMERA_FOCUS_MACRO, 1, R.string.menu_camera_focus_macro);
		}
		tmp = camera.getParameters().getSupportedSceneModes();
		if(tmp != null)
		{
			SubMenu subScene = menu.addSubMenu(0, MENU_CAMERA_SCENE, 2, R.string.menu_camera_scene);
			if(tmp.contains(Parameters.SCENE_MODE_AUTO))
				subScene.add(0, MENU_CAMERA_SCENE_AUTO, 2, R.string.menu_camera_scene_atuo);
			if(tmp.contains(Parameters.SCENE_MODE_ACTION))
				subScene.add(0, MENU_CAMERA_SCENE_ACTION, 2, R.string.menu_camera_scene_action);
			if(tmp.contains(Parameters.SCENE_MODE_PORTRAIT))
				subScene.add(0, MENU_CAMERA_SCENE_PORTRAIT, 2, R.string.menu_camera_scene_portrait);
			if(tmp.contains(Parameters.SCENE_MODE_LANDSCAPE))
				subScene.add(0, MENU_CAMERA_SCENE_LANDSCAPE, 2, R.string.menu_camera_scene_landscape);
			if(tmp.contains(Parameters.SCENE_MODE_NIGHT))
				subScene.add(0, MENU_CAMERA_SCENE_NIGHT, 2, R.string.menu_camera_scene_night);
			if(tmp.contains(Parameters.SCENE_MODE_NIGHT_PORTRAIT))
				subScene.add(0, MENU_CAMERA_SCENE_NIGHPORTRAIT, 2, R.string.menu_camera_scene_nightportrait);
			if(tmp.contains(Parameters.SCENE_MODE_THEATRE))
				subScene.add(0, MENU_CAMERA_SCENE_THEATRE, 2, R.string.menu_camera_scene_theatre);
			if(tmp.contains(Parameters.SCENE_MODE_BEACH))
				subScene.add(0, MENU_CAMERA_SCENE_BEACH, 2, R.string.menu_camera_scene_beach);
			if(tmp.contains(Parameters.SCENE_MODE_SNOW))
				subScene.add(0, MENU_CAMERA_SCENE_SNOW, 2, R.string.menu_camera_scene_snow);
			if(tmp.contains(Parameters.SCENE_MODE_SUNSET))
				subScene.add(0, MENU_CAMERA_SCENE_SUNSET, 2, R.string.menu_camera_scene_sunset);
			if(tmp.contains(Parameters.SCENE_MODE_STEADYPHOTO))
				subScene.add(0, MENU_CAMERA_SCENE_STEADYPHOTO, 2, R.string.menu_camera_scene_steadyphoto);
			if(tmp.contains(Parameters.SCENE_MODE_FIREWORKS))
				subScene.add(0, MENU_CAMERA_SCENE_FIREWORKS, 2, R.string.menu_camera_scene_fireworks);
			if(tmp.contains(Parameters.SCENE_MODE_SPORTS))
				subScene.add(0, MENU_CAMERA_SCENE_SPORTS, 2, R.string.menu_camera_scene_sports);
			if(tmp.contains(Parameters.SCENE_MODE_PARTY))
				subScene.add(0, MENU_CAMERA_SCENE_PARTY, 2, R.string.menu_camera_scene_party);
			if(tmp.contains(Parameters.SCENE_MODE_CANDLELIGHT))
				subScene.add(0, MENU_CAMERA_SCENE_CANDLELIGHT, 2, R.string.menu_camera_scene_candlelight);
		}
		tmp = camera.getParameters().getSupportedWhiteBalance();
		if(tmp != null)
		{
			SubMenu subWhiteBal = menu.addSubMenu(0, MENU_CAMERA_WHITEBALANCE, 2, R.string.menu_camera_whitebalance);
			if(tmp.contains(Parameters.WHITE_BALANCE_AUTO))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_AUTO, 2, R.string.menu_camera_whitebalance_auto);
			if(tmp.contains(Parameters.WHITE_BALANCE_INCANDESCENT))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_INCANDESCENT, 2, R.string.menu_camera_whitebalance_incandescent);
			if(tmp.contains(Parameters.WHITE_BALANCE_FLUORESCENT))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_FLUORESCENT, 2, R.string.menu_camera_whitebalance_fluorescent);
			if(tmp.contains(Parameters.WHITE_BALANCE_DAYLIGHT))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_DAYLIGHT, 2, R.string.menu_camera_whitebalance_daylight);
			if(tmp.contains(Parameters.WHITE_BALANCE_CLOUDY_DAYLIGHT))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_CLOUDYDAYLIGHT, 2, R.string.menu_camera_whitebalance_cloudydaylight);
			if(tmp.contains(Parameters.WHITE_BALANCE_TWILIGHT))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_TWILIGHT, 2, R.string.menu_camera_whitebalance_twilight);
			if(tmp.contains(Parameters.WHITE_BALANCE_SHADE))
				subWhiteBal.add(0, MENU_CAMERA_WHITEBALANCE_SHADE, 2, R.string.menu_camera_whitebalance_shade);
		}
		SubMenu subQuality = menu.addSubMenu(0, MENU_CAMERA_QUALITY, 3, R.string.menu_camera_quality);
		subQuality.add(0, MENU_CAMERA_QUALITY_LOW, 3, R.string.menu_camera_quality_low);
		subQuality.add(0, MENU_CAMERA_QUALITY_MEDIUM, 3, R.string.menu_camera_quality_medium);
		subQuality.add(0, MENU_CAMERA_QUALITY_HIGH, 3, R.string.menu_camera_quality_high);
		subQuality.add(0, MENU_CAMERA_QUALITY_VERYHIGH, 3, R.string.menu_camera_quality_veryhigh);
	 ...
	 * @param camera				obiekt aparatu, dla ktorego wprowadzone zostana zmiany
	 * @param photoQuality			obecna jakosc zdjecia
	 * @param inPreview				czy wlaczony jest podglad z aparatu
	 * @param isCameraViewSet		ktory widok jest aktualnie aktywny
	 * @param lCameraPreview		widok zawierajacy podglad z aparatu
	 */
	public static void setParameter(int parameterId, Camera camera, int photoQuality, boolean inPreview, int isCameraViewSet, LinearLayout lCameraPreview) {
		switch(parameterId)
		{
	...
	private static void setScene(String scene, Camera camera, boolean inPreview, int isCameraViewSet, LinearLayout lCameraPreview)
	{
		if(inPreview && lCameraPreview.getVisibility()==View.VISIBLE && isCameraViewSet == 0)
		{
			camera.stopPreview();
			inPreview = false;
		}
	...
	private static void setWhiteBalance(String whBalance, Camera camera, boolean inPreview, int isCameraViewSet, LinearLayout lCameraPreview)
	{
		if(inPreview && lCameraPreview.getVisibility()==View.VISIBLE && isCameraViewSet == 0)
		{
			camera.stopPreview();
	...
	private static void setFlash(String flash, Camera camera, boolean inPreview, int isCameraViewSet, LinearLayout lCameraPreview)
	{
		if(inPreview && lCameraPreview.getVisibility()==View.VISIBLE && isCameraViewSet == 0)
		{
			camera.stopPreview();
	...
	private static void setFocus(String focus, Camera camera, boolean inPreview, int isCameraViewSet, LinearLayout lCameraPreview)
	{
		if(inPreview && lCameraPreview.getVisibility()==View.VISIBLE && isCameraViewSet == 0)
		{
			camera.stopPreview();
